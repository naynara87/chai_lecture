...
const Dialog = ({
  ...
  const [userSelectedMultiChoices, setUserSelectedMultiChoices] = useState<
    MultiChoice[]
  >([]);
  const [userSelectedIndexes, setUserSelectedIndexes] = useState<
    (number | undefined)[]
  >([]);
  ...
  const handleClickTextBox = useCallback(
    (choice: MultiChoice, index: number) => {
      ...
      const emptyIndex = userSelectedIndexes.findIndex((SelectedIndex) => {
          ...
          {sortChoiceList.map((choice, index) => {
            return (
              <AnswerChoice
                ...
                inCorrectColor={
                  isShowCorrect === undefined || isShowCorrect
                    ? inCorrectColor
                    : correctColor
                }
  ...
  const isMultiChoicesUserAnswerFull = useMemo(() => {
    return (
      userSelectedMultiChoices.length > 0 &&
      userSelectedMultiChoices.length >= questions.length
    );
