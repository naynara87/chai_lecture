...
const TP11EComponent = ({
  ...
  const iconTextContentData = useMemo(() => {
    return thisPage.template.contents.find(
      (content) => content.type === "iconText",
    ) as IconTextContent | undefined;
  }, [thisPage.template.contents]);

  const chooseTextContentData = useMemo(() => {
    return thisPage.template.contents.find(
      (content) => content.type === "chooseText",
    ) as ChooseTextContent | undefined;
  }, [thisPage.template.contents]);
  ...
  return (
    <TemplateCommonLayout>
      ...
      <TP11Layout>
        <IconText
          text={iconTextContentData?.data?.[0].text ?? ""}
          customCss={iconTextCss}
        />
        {chooseTextContentData ? (
          <ChooseText contentData={chooseTextContentData} />
        ) : (
          <></>
        )}
      </TP11Layout>
