...
const TP04AComponent = ({
  ...
  const renderContents = useMemo(() => {
    ...
    const minLengthContent =
      htmlContents.length <= imageContents.length
        ? htmlContents
        : imageContents;
    return minLengthContent.map((_, index) => {
      return (
        <ContainerWrapper key={index}>
          ...
          {htmlContents[index] && (
            <HtmlContentComponent
              html={htmlContents[index].text}
              customCss={htmlCustomCss}
            />
          )}
        </ContainerWrapper>
      );
